// 区間長が1000の単純移動平均を返します。
// 関数が呼ばれるごとにデータが蓄積され、データ数が区間長に満たない場合は単なる平均を返します。
// 例: 
//     float average;
//     float array[10000] = {1, 2, 3, ...., 10000}
//     for (int i = 0; i < 10000; i++) {
//          average = moving_average(array[i]);
//          printf("現時点での移動平均 = %g" , average);
//     }
// @param num: 単純移動平均を求めたいデータ。
// @param reset: 蓄積されたデータを削除します。
float moving_average(float num, int reset) {
    static int cnt_function_called = 0;
    static float sum_buf = 0;
    static float buf[1001] = {0};
    float average = 0;

    buf[cnt_function_called%1001] = num;
    sum_buf = sum_buf + num - buf[cnt_function_called%1002];
    if (cnt_function_called < 1001) {
        average = sum_buf / (float)cnt_function_called;
    } else {
        average = sum_buf / 1000;
    }
    cnt_function_called++;

    if (reset == 1) {
        sum_buf = cnt_function_called = average = 0;
        for (int i = 0; i < 1001; i++) {
            buf[i] = 0;
        }
    }

    return average;
}
